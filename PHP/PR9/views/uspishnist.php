		...
		<form method="POST" action="/index.php/uspishnist/actions">
			<table>
				...
				<? foreach ($uspishnist as $u) { ?>
					<tr>
						<td> <input type="number" name="mark[<? echo $u['id']; ?>]" placeholder="Mark" required value="<? echo $u['mark']; ?>"> </td>
						<td>
							<select name="s_id[<?php echo $u['id']; ?>]">
								<option value="">None</option>
								<?php if ($students) { ?>
									<? foreach ($students as $s) { ?>
										<option value="<?php echo $s['id']; ?>" <?php if ($u['s_id'] == $s['id']) echo 'selected';  ?>><?php echo $s['name']; ?></option>
									<? } ?>
								<? } ?>
							</select>
						</td>
						<td>
							<select name="p_id[<?php echo $u['id']; ?>]">
								...
								<?php if ($subjects) { ?>
									<? foreach ($subjects as $s) { ?>
										<option value="<?php echo $s['id']; ?>" <?php if ($u['p_id'] == $s['id']) echo 'selected';  ?>><?php echo $s['name']; ?></option>
									<? } ?>
								<? } ?>
							</select>
						</td>
